...
private def tokenHover : Token Highlighted → Option String
  | ⟨.const _ sig doc? _, _⟩ => some (mkHover sig.toString doc?) -- Use toHtml for signature? No, sig is Highlighted.
  | ⟨.var _ ty, s⟩ => some (mkHover s!"{s} : {ty.toString}" none) -- ty is Highlighted?
  | _ => none
...
private partial def findTrace? (header : String) : MessageContents Highlighted Highlighted → Option (MessageContents Highlighted Highlighted)
  | .text .. | .term .. | .goal .. => none
  | .append xs =>
