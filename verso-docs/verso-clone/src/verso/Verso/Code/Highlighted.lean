...
public defmethod Token.Kind.addLink (tok : Token.Kind Highlighted) (content : Html) : HighlightHtmlM g Html := do
...
public defmethod Token.Kind.«class» : Token.Kind Highlighted → String
  | .var .. => "var"
  | .str .. => "literal string"
  | .sort .. => "sort"
  | .const .. => "const"
...
public defmethod Token.Kind.data : Token.Kind Highlighted → String
...
public defmethod Token.Kind.idAttr : Token.Kind Highlighted → HighlightHtmlM g (Array (String × String))
  | .const n _ _ true => do
...
-/
defmethod Token.htmlContent (tok : Token Highlighted) : HighlightHtmlM g Html := do
...
mutual
  partial def Highlighted.MessageContents.toHtml (expandTraces : List Lean.Name) (maxTraceDepth : Nat) (exprHtml : expr → HighlightHtmlM g Html) : Highlighted.MessageContents Highlighted expr → HighlightHtmlM g Html
  ...
  | .append xs => xs.foldlM (init := Html.empty) fun html m =>
      (html ++ ·) <$> Highlighted.MessageContents.toHtml expandTraces maxTraceDepth exprHtml m
  | .trace cls msg children collapsed => do
    let msgHtml ← Highlighted.MessageContents.toHtml expandTraces maxTraceDepth exprHtml msg
    let collapsed := collapsed && cls ∉ expandTraces
    let childHtml ←
      if maxTraceDepth = 0 then pure Html.empty
      else
        let cs ← children.mapM (Highlighted.MessageContents.toHtml expandTraces (maxTraceDepth - 1) exprHtml)
  ...
  | .goal g => Highlighted.Goal.toHtml exprHtml 0 g

  partial def Highlighted.Goal.toHtml (exprHtml : expr → HighlightHtmlM g Html) (index : Nat) : Highlighted.Goal Highlighted expr → HighlightHtmlM g Html
          ...
          {{← hypotheses.mapM fun
              | ⟨names, t⟩ => do pure {{
                  <span class="hypothesis">
                    <span class="name">{{(← names.mapM Token.toHtml).toList.intersperse {{" "}} }}</span><span class="colon">":"</span>
  ...
  partial def Token.Kind.hover? (tok : Token.Kind Highlighted) : HighlightHtmlM g (Option Nat) :=
    match tok with
    | .const _n sig doc _ | .anonCtor _n sig doc => do
      let docs :=
        match doc with
        | none => .empty
        | some txt => separatedDocs txt
      let sigHtml ← Highlighted.toHtml sig
      let html := {{ <code>{{sigHtml}}</code> {{docs}} }}
      some <$> addHover html
    | .option optName _declName doc => do
      let docs := match doc with
        | none => .empty
        | some txt => separatedDocs txt
      let html := {{ <code>{{toString optName}}</code> {{docs}} }}
      some <$> addHover html
    | .keyword _ _ none => pure none
    | .keyword _ _ (some doc) => some <$> addHover {{<code class="docstring">{{doc}}</code>}}
    | .var _ type => do
      let typeHtml ← Highlighted.toHtml type
      let html := {{ <code>{{typeHtml}}</code> }}
      some <$> addHover html
    | .str s =>
      some <$> addHover {{ <code><span class="literal string">{{s.quote}}</span>" : String"</code>}}
    | .withType t => do
      let tHtml ← Highlighted.toHtml t
      let html := {{ <code>{{tHtml}}</code> }}
      some <$> addHover html
    | .sort (some doc) =>
  ...
  partial def Token.toHtml (tok : Token Highlighted) : HighlightHtmlM g Html := do
    let hoverId ← Token.Kind.hover? tok.kind
    let idAttr ← Token.Kind.idAttr tok.kind
    let hoverAttr := hoverId.map (fun i => #[("data-verso-hover", toString i)]) |>.getD #[]
    Token.Kind.addLink tok.kind {{
      <span class={{(Token.Kind.«class» tok.kind) ++ " token"}} data-binding={{Token.Kind.data tok.kind}} {{hoverAttr}} {{idAttr}}>{{← tok.htmlContent}}</span>
    }}

  partial def Highlighted.toHtml : Highlighted → HighlightHtmlM g Html
    | .token t => Token.toHtml t
            ...
            <span class="hover-container">
              <span class={{"hover-info messages"}}>
                {{←  infos.mapM fun (s, info) => do return {{
                  <code class={{"verso-message " ++ s.«class»}}>{{← Highlighted.MessageContents.toHtml [] 10 Highlighted.toHtml info}}</code> }}
                }}
              </span>
            </span>
            {{← Highlighted.toHtml hl}}
          </span>
        }}
      else
        panic! "No highlights!"
    ...
    | .tactics info startPos endPos hl => do
      if (← options).inlineProofStates then
        ...
        pure {{
          <span class="tactic">
            <label for={{id}}>{{← Highlighted.toHtml hl}}</label>
            <input type="checkbox" class="tactic-toggle" id={{id}} {{checkedAttr}}/>
            <span class="tactic-state">
              {{← if info.isEmpty then
                  pure {{"All goals completed! 🐙"}}
                else
                  .seq <$> info.mapIdxM (fun i x => Highlighted.Goal.toHtml Highlighted.toHtml i x)}}
            </span>
          </span>
        }}
      else
        Highlighted.toHtml hl
    | .point s info => do
      let info ← Highlighted.MessageContents.toHtml [] 10 Highlighted.toHtml info
      return {{
        <span class={{"verso-message " ++ s.«class»}}>{{info}}</span>
      }}
    | .seq hls => hls.mapM Highlighted.toHtml
end
...
public defmethod Highlighted.blockHtml (contextName : String) (code : Highlighted) (trim : Bool := true) (htmlId : Option String := none) : HighlightHtmlM g Html := do
  ...
  pure {{ <code class="hl lean block" "data-lean-context"={{toString contextName}} {{idAttr}}> {{ ← Highlighted.toHtml code }} </code> }}
...
public defmethod Highlighted.inlineHtml (contextName : Option String) (code : Highlighted) (trim : Bool := true) (htmlId : Option String := none) : HighlightHtmlM g Html := do
  ...
  if let some ctx := contextName then
    pure {{ <code class="hl lean inline" "data-lean-context"={{toString ctx}} {{idAttr}}> {{ ← Highlighted.toHtml code }} </code> }}
  else
    pure {{ <code class="hl lean inline" {{idAttr}}> {{ ← Highlighted.toHtml code }} </code> }}
...
public defmethod Highlighted.Message.toHtml (message : Highlighted.Message) (expandTraces : List Lean.Name) (maxTraceDepth : Nat := 10) : HighlightHtmlM g Html := do
  let contents ← Highlighted.MessageContents.toHtml expandTraces maxTraceDepth (Highlighted.toHtml) message.contents
...
window.onload = () => {
    ...
    // Add hovers
    // Add hovers
    const versoDocData = window.versoDocData || {};
    if (versoDocData) {
      ...
      const defaultTippyProps = {
        ...
        maxWidth: \"none\",
        appendTo: (ref) => {
          const parentBox = ref.closest('.tippy-box');
          return parentBox || document.body;
        },
        interactive: true,
          ...
          if (tgt.className == 'tactic') {
            ...
            if (hoverId) { // Docstrings from the table
              ...
              if (data) {
                ...
                const nestedTokens = content.querySelectorAll(nestedTokenSelector);
                console.log(\"Recursive hover debug:\", { hoverId, dataLen: data.length, nestedTokens: nestedTokens.length });
                tippy(nestedTokens, {
                  ...defaultTippyProps,
      ...
      const nestedTokenSelector = '.token'; // Simpler selector for nested tokens

      document.querySelectorAll(tokenSelector).forEach(element => {
      ...
      let insts = tippy(tokenSelector, defaultTippyProps);

      // Dismiss hovers on Escape
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            if (typeof tippy.hideAll === 'function') {
                tippy.hideAll();
            } else {
                document.querySelectorAll(tokenSelector).forEach(el => {
