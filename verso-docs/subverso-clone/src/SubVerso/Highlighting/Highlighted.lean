...
inductive Token.Kind (signature : Type) where
  ...
  | const (name : Name) (sig : signature) (docs : Option String) (isDef : Bool)
  | anonCtor (name : Name) (sig : signature) (docs : Option String)
  | var (name : FVarId) (type : signature)
  | str (string : String)
  ...
  | /-- The token represents some otherwise-undescribed Expr whose type is known -/
    withType (type : signature)
  | unknown
deriving Repr, Inhabited, BEq, Hashable, ToJson, FromJson

def Token.Kind.priority : Token.Kind signature → Nat
  | .var .. => 1
  | .const .. => 2
  | .option .. => 3
  | .keyword .. => 4
...
instance [Quote signature] : Quote (Token.Kind signature) where
  quote
...
structure Token (signature : Type) where
  kind : Token.Kind signature
  content : String
deriving Repr, Inhabited, BEq, Hashable, ToJson, FromJson

open Syntax in
instance [Quote signature] : Quote (Token signature) where
  quote
    | (.mk kind content) =>
      mkCApp ``Token.mk #[quote kind, quote content]

structure Highlighted.Hypothesis (signature : Type) (expr : Type) where
  names : Array (Token signature)
  typeAndVal : expr
deriving Repr, BEq, Hashable, ToJson, FromJson

def Highlighted.Hypothesis.map (f : α → β) (g : γ → δ) (h : Hypothesis α γ) : Hypothesis β δ :=
  { h with
    names := h.names.map (fun t => {t with kind := match t.kind with
      | .const n s d i => .const n (f s) d i
      | .anonCtor n s d => .anonCtor n (f s) d
      | .var n t => .var n (f t)
      | .withType t => .withType (f t)
      | .keyword n o d => .keyword n o d
      | .str s => .str s
      | .option n d docs => .option n d docs
      | .docComment => .docComment
      | .sort d => .sort d
      | .levelVar n => .levelVar n
      | .levelOp w => .levelOp w
      | .levelConst i => .levelConst i
      | .moduleName n => .moduleName n
      | .unknown => .unknown
    })
    typeAndVal := g h.typeAndVal }

structure Highlighted.Goal (signature : Type) (expr : Type) where
  name : Option String
  goalPrefix : String
  hypotheses : Array (Hypothesis signature expr)
  conclusion : expr
deriving Repr, BEq, Hashable, ToJson, FromJson

def Highlighted.Goal.map (f : α → β) (g : γ → δ) (goal : Goal α γ) : Goal β δ :=
  { goal with
    hypotheses := goal.hypotheses.map (·.map f g)
    conclusion := g goal.conclusion }

instance [Quote signature] [Quote expr] : Quote (Highlighted.Hypothesis signature expr) where
  quote
    | {names, typeAndVal} =>
      Syntax.mkCApp ``Highlighted.Hypothesis.mk #[quote names, quote typeAndVal]

instance [Quote signature] [Quote expr] : Quote (Highlighted.Goal signature expr) where
  quote
...
inductive Highlighted.MessageContents (signature : Type) (expr : Type) where
  | text : String → MessageContents signature expr
  | goal : Goal signature expr → MessageContents signature expr
  | term : expr → MessageContents signature expr
  | trace (cls : Name) (msg : MessageContents signature expr) (children : Array (MessageContents signature expr)) (collapsed : Bool) : MessageContents signature expr
  | append : Array (MessageContents signature expr) → MessageContents signature expr
deriving Repr, Inhabited, BEq, Hashable, ToJson, FromJson

open Highlighted MessageContents in
open Syntax in
partial instance [Quote signature] [Quote expr] : Quote (MessageContents signature expr) where
  quote := q
where
  q
    ...
    | .trace cls msg children collapsed =>
      have : Quote (MessageContents signature expr) := ⟨q⟩
...
inductive Highlighted where
  | token (tok : Token Highlighted)
  | text (str : String)
  | seq (highlights : Array Highlighted)
  | span (info : Array (Span.Kind × MessageContents Highlighted Highlighted)) (content : Highlighted)
  | tactics (info : Array (Goal Highlighted Highlighted)) (startPos : Nat) (endPos : Nat) (content : Highlighted)
  | point (kind : Span.Kind) (info : MessageContents Highlighted Highlighted)
  | unparsed (str : String)
deriving Repr, Inhabited, BEq, Hashable, ToJson, FromJson

structure Highlighted.Message where
  severity : Span.Kind
  contents : MessageContents Highlighted Highlighted
deriving Repr, Inhabited, BEq, Hashable, ToJson, FromJson
...
-/
partial def Goal.toString : Highlighted.Goal Highlighted Highlighted → String
  | {name, goalPrefix, hypotheses, conclusion} =>
    (name.map ("case " ++ · ++ "\n") |>.getD "") ++
...
partial def MessageContents.toString (expandTraces : List Name := []) : MessageContents Highlighted Highlighted → String
  | .trace cls msg children collapsed =>
...
-/
partial def MessageContents.startAt (pattern : String) (txt : MessageContents sig expr) : Option (MessageContents sig expr) := do
  let mut todo := [txt]
  let mut pat := pattern
...
-/
partial def MessageContents.stopAt (pattern : String) (txt : MessageContents sig expr) : Option (MessageContents sig expr) := do
  let mut todo := [txt]
  let mut pat := pattern
  let mut strict : Bool := false
