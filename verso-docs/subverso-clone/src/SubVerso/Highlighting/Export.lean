...
structure Export where
  ...
  tokens : Compat.HashMap Export.Key (Token Highlighted) := {}
...
structure Exporting extends Export where
  tokenKeys : Compat.HashMap (Token Highlighted) Export.Key := {}
...
def Token.export (tok : Token Highlighted) : ExportM Export.Key := do
...
mutual
  partial def Highlighted.MessageContents.export (txt : Highlighted.MessageContents Highlighted Highlighted) : ExportM Export.Key := do
  ...
  partial def Highlighted.Hypothesis.export (hyp : Highlighted.Hypothesis Highlighted Highlighted) : ExportM Export.Hypothesis := do
    return { names := (← hyp.names.mapM (·.export)), typeAndVal := (← hyp.typeAndVal.export) }

  partial def Highlighted.Goal.export (goal : Highlighted.Goal Highlighted Highlighted) : ExportM Export.Key := do
...
mutual
  partial def Export.toToken (data : Export) (key : Key) : Except String (Highlighting.Token Highlighting.Highlighted) := do
    let some tok := data.tokens.get? key
      | throw s!"Not found: token at key {key}"
    pure tok

  partial def Export.toMessageContents (data : Export) (key : Key) : Except String (Highlighted.MessageContents Highlighting.Highlighted Highlighting.Highlighted) := do
  ...
  partial def Export.toGoal (data : Export) (key : Key) : Except String (Highlighted.Goal Highlighting.Highlighted Highlighting.Highlighted) := do
    let some g := data.goals.get? key
      | throw s!"Not found: goal at key {key}"
    let hypotheses : Array (Highlighted.Hypothesis Highlighting.Highlighted Highlighting.Highlighted) ← g.hypotheses.mapM fun {names, typeAndVal} => do
