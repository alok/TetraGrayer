...
partial def bestD (sems : Array (Token.Kind Highlighted)) (default : Token.Kind Highlighted) : Token.Kind Highlighted :=
  if let some m := sems.back? then
    bestD sems.pop <| if m.priority > default.priority then m else default
  else
    default

def fakeToken (meaning : Token.Kind Highlighted) (tok : String) : Token Highlighted where
  content := tok
  kind := meaning
...
partial def stripNamespacesText [Monad m] [MonadReaderOf Context m] (tt : TaggedText α) : m (TaggedText α) := do
  match tt with
  | .text str =>
    for i in (← read).suppressNamespaces do
      let ns := i.toString ++ "."
      if ns.isPrefixOf str then
        return .text (str.drop ns.length)
    pure <| .text str
  | .tag t tt' => .tag t <$> stripNamespacesText tt'
  | .append tts => .append <$> tts.mapM stripNamespacesText
...
def exprKind [Monad m] [MonadLiftT IO m] [MonadMCtx m] [MonadEnv m] [Alternative m]
    ...
    (render : CodeWithInfos → ReaderT Context m Highlighted)
    (allowUnknownTyped : Bool := false) :
    ReaderT Context m (Option (Token.Kind Highlighted)) := do
  ...
  -- qualifiers
  let ppSig (x : Name) (env := ci.env) : ReaderT Context m Highlighted := do
    let type ← runMeta (env := env) (lctx := {}) do
      let info ← getConstInfo x
      instantiateMVars info.type
    let code ← runMeta (env := env) (lctx := {}) (ppCodeWithInfos type)
    let code ← stripNamespacesText code
    render code
  ...
  let rec findKind e := do
    ...
    | Expr.sort _ =>
      if let some stx := stx? then
        let k := stx.getKind
        if k == `Lean.Parser.Term.type then
          return some (.sort (some "The universe of types."))
    ...
    | other =>
      ...
      if allowUnknownTyped then
        let hl? ← runMeta do
          try
            let t ← Meta.inferType other >>= instantiateMVars
            some <$> ppCodeWithInfos t
          catch _ =>
            pure none
        if let some hl := hl? then
          let hl ← render hl
          return some <| .withType hl
        else return none
      else
        return none
...
def termInfoKind [Monad m] [MonadLiftT IO m] [MonadMCtx m] [MonadEnv m] [MonadFileMap m] [Alternative m]
    ...
    (allowUnknownTyped : Bool := false) :
    ReaderT Context m (Option (Token.Kind Highlighted)) := do
  let k ← exprKind ci termInfo.lctx termInfo.stx termInfo.expr render (allowUnknownTyped := allowUnknownTyped)
...
def fieldInfoKind [Monad m] [MonadMCtx m] [MonadLiftT IO m] [MonadEnv m]
    ...
    (render : CodeWithInfos → ReaderT Context m Highlighted) :
    ReaderT Context m (Token.Kind Highlighted) := do
  let runMeta {α} (act : MetaM α) : m α := ci.runMetaM fieldInfo.lctx act
  let ty ← runMeta
    try
      let valTy ← instantiateMVars (← Meta.inferType fieldInfo.val)
      ppCodeWithInfos valTy
    catch
      | _ =>
        try
          let fmt ← PrettyPrinter.ppSignature fieldInfo.projName
          pure <| .text fmt.1.pretty
        catch _ => pure <| .text ""
  let tyStr ← render ty
...
def infoKind [Monad m] [MonadLiftT IO m] [MonadMCtx m] [MonadEnv m] [MonadFileMap m] [Alternative m]
    ...
    ReaderT Context m (Option (Token.Kind Highlighted)) := do
  match info with
    | .ofTermInfo termInfo => termInfoKind ci termInfo render (allowUnknownTyped := allowUnknownTyped)
    | .ofFieldInfo fieldInfo => some <$> fieldInfoKind ci fieldInfo render
    | .ofOptionInfo oi =>
...
def identKind [Monad m] [MonadLiftT IO m] [MonadFileMap m] [MonadEnv m] [MonadMCtx m] [Alternative m]
    ...
    (allowUnknownTyped : Bool := false) :
    ReaderT Context m (Token.Kind Highlighted) := do
  let mut kind : Token.Kind Highlighted := .unknown
  for t in trees do
    for (ci, info) in infoForSyntax t stx do
      if let some seen ← infoKind ci info render (allowUnknownTyped := allowUnknownTyped) then
...
def anonCtorKind [Monad m] [MonadLiftT IO m] [MonadFileMap m] [MonadEnv m] [MonadMCtx m] [Alternative m]
  ...
  for t in trees do
    for (ci, info) in infoForSyntax t stx do
      if let some seen ← infoKind ci info render then
        if seen.priority > kind.priority then kind := seen
      else continue
...
inductive Output where
  ...
  | tactics (info : Array (Goal Highlighted Highlighted)) (startPos : Compat.String.Pos) (endPos : Compat.String.Pos)
deriving Repr
...
def Output.addToken (output : List Output) (token : Token Highlighted) : List Output :=
...
def Output.inTacticState (output : List Output) (info : Array (Highlighted.Goal Highlighted Highlighted)) : Bool :=
  output.any fun
...
def HighlightState.openTactic (st : HighlightState) (info : Array (Highlighted.Goal Highlighted Highlighted)) (startPos endPos : Compat.String.Pos) (_pos : Position) : HighlightState :=
...
def HighlightM.openTactic (info : Array (Highlighted.Goal Highlighted Highlighted)) (startPos endPos : Compat.String.Pos) (pos : Position) : HighlightM Unit :=
...
partial def renderTagged [Monad m] [MonadLiftT IO m] [MonadMCtx m] [MonadEnv m] [MonadFileMap m] [Alternative m]
    (outer : Option (Token.Kind Highlighted)) (doc : CodeWithInfos) :
  ...
  | .tag t doc' =>
    ...
    if let .text tok := doc' then
      ...
      let k := (← infoKind ctx info (renderTagged none)).getD .unknown
      pure <| .seq #[.text wsPre, .token ⟨k, tok.trim⟩, .text wsPost]
    else
      let k? ← infoKind ctx info (renderTagged none)
      renderTagged k? doc'
...
partial def messageContents
    ...
    HighlightM (Highlighted.MessageContents Highlighted Highlighted) := do
...
where
  convert (txt : TaggedText MsgEmbed) : HighlightM (Highlighted.MessageContents Highlighted Highlighted) :=
    match txt with
    ...
    | .tag embed _ =>
      let render (e : CodeWithInfos) : HighlightM Highlighted  :=
        if highlightTerms then
          ...
          pure <| .text e.pretty
      Compat.msgEmbedCase (α := HighlightM (Highlighted.MessageContents Highlighted Highlighted)) embed
        (onExpr := fun e =>
          if let some txt := codeWithInfosIsString? e then pure <| .text txt
          else .term <$> render e)
        (onGoal := fun g => do
          let mut hypotheses : Array (Highlighted.Hypothesis Highlighted Highlighted) := #[]
          for h in g.hyps do
            -- TODO mvar ctx?
            let mut names : Array (Token Highlighted) := #[]
            let t ← render h.type
...
def emitToken (blame : Syntax) (info : SourceInfo) (token : Token Highlighted) : HighlightM Unit := do
...
def emitToken' (token : Token Highlighted) : HighlightM Unit := do
  modify fun st => {st with output := Output.addToken st.output token}
...
def highlightGoals (ci : ContextInfo) (goals : List MVarId) :
    HighlightM (Array (Highlighted.Goal Highlighted Highlighted)) := withReader (·.noDefinitions) do
  let mut goalView := #[]
  for g in goals do
    let mut hyps := #[]
    ...
    for c in lctx.decls do
      ...
      | .cdecl _index fvar name type _ _ =>
        let nk ← exprKind ci lctx none (.fvar fvar) (renderTagged none)
...
def tacticInfoGoals
    ...
    (before : Bool := false) :
    HighlightM (Array (Highlighted.Goal Highlighted Highlighted) × Compat.String.Pos × Compat.String.Pos × Position) := do
...
partial def findTactics'
    ...
    (endPosition : Position)
    (before : Bool := false)
    : HighlightM (Option (Array (Highlighted.Goal Highlighted Highlighted) × Compat.String.Pos × Compat.String.Pos × Position)) := do
...
partial def findTactics
    ...
    (stx : Syntax)
    (before : Bool := false)
    : HighlightM (Option (Array (Highlighted.Goal Highlighted Highlighted) × Compat.String.Pos × Compat.String.Pos × Position)) :=
...
-/
def beforeGoals (stx : Syntax) : HighlightM (Option (Array (Highlighted.Goal Highlighted Highlighted))) := do
  let text ← getFileMap
...
partial def highlight'
  ...
  highlightSpecial trees stx tactics (lookingAt := lookingAt) fun trees tactics lookingAt => fun
        ...
        | .str (.str _ _) _ =>
          ...
          | [y, field] =>
            ...
            if (← infoExists trees field) then
              ...
              let k ← identKind trees ⟨stx⟩ (renderTagged none)
    ...
    | stx@(.atom i x) =>
      ...
      if let .sort docs? ← identKind trees ⟨stx⟩ (renderTagged none) then
    ...
    | .node _ `num #[.atom i n] =>
      withTraceNode `SubVerso.Highlighting.Code (fun _ => pure m!"Numeral") do
        let k ← identKind trees ⟨stx⟩ (renderTagged none) (allowUnknownTyped := true)
        emitToken stx i ⟨k, n⟩
    ...
    | .node _ ``Lean.Parser.Term.dotIdent #[dot@(.atom i _), name@(.ident i' _ x _)] =>
      ...
      | .original leading pos _ _, .original _ _ trailing endPos =>
        ...
        emitToken stx info ⟨← identKind trees ⟨stx⟩ (renderTagged none), s!".{x.toString}"⟩
      | _, _ =>
    ...
    | .node _ k@``Lean.Parser.Term.anonymousCtor #[opener@(.atom oi l), children@(.node _ _ contents), closer@(.atom ci r)] => do
      if let some tk ← anonCtorKind trees stx (renderTagged none) then
...
def highlightProofState (ci : ContextInfo) (goals : List MVarId)
    (trees : PersistentArray Lean.Elab.InfoTree)
    (suppressNamespaces : List Name := []) : TermElabM (Array (Highlighted.Goal Highlighted Highlighted)) := do
  let trees := trees.toArray
  let modrefs := Lean.Server.findModuleRefs (← getFileMap) trees
  let ids := build modrefs
  let infoTable : InfoTable := .ofInfoTrees trees
