...
def copysign (x y : ℝ) : ℝ :=
  let absX := Float.abs x
  if y >= 0.0 then absX else -absX

/-- Maximum of two floats. -/
def fmax (x y : ℝ) : ℝ := if x >= y then x else y

/-- Convert Cartesian position (t,x,y,z) to spherical (t,r,θ,φ). -/
def sphericalFromCartesian (pos : CliffordVector) : CliffordVector :=
  let t := pos.v0
  let x := pos.v1
  let y := pos.v2
  let z := pos.v3
...
def spheroidalFromCartesian (a : ℝ) (pos : CliffordVector) : CliffordVector :=
  let t := pos.v0
  let x := pos.v1
  let y := pos.v2
  let z := pos.v3
...
def spheroidalBasisEmu (sinhMu coshMu sinNu cosNu sinPhi cosPhi : ℝ) : CliffordVector :=
...
def spheroidalBasisEnu (sinhMu coshMu sinNu cosNu sinPhi cosPhi : ℝ) : CliffordVector :=
  let denom := Float.sqrt (sinhMu * sinhMu + cosNu * cosNu)
  let invD := 1.0 / denom
...
def spheroidalBasisPhi (sinPhi cosPhi : ℝ) : CliffordVector :=
  CliffordVector.mk4 0.0 (-sinPhi) cosPhi 0.0

/-- Spheroidal basis vector (time direction). -/
def spheroidalBasisT : CliffordVector :=
